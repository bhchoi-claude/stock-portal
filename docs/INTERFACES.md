...
**만료 전에도 토큰이 죽는다** (2026-09-02 실측). 키움은 **앱키당 토큰
하나만** 유효하게 한다. 같은 앱키로 새로 발급하면 이전 토큰이 무효화된다.

```
08:55  실험이 토큰 발급
16:10  키움 수집 유닛이 같은 앱키로 발급
16:12  실험의 호출 -> 인증에 실패했습니다[8005:Token이 유효하지 않습니다]
```

시세 수집기가 전부 `use_paper: true` 라 **모의투자 앱키 하나를 공유한다.**
24시간 도는 엔진도 같은 앱키를 쓴다.

그래서 본문 `return_code = 3`(인증 실패)은 **`TransientError` 로 분류하고
캐시한 토큰을 버린다.** 조회는 `_call` 의 재시도로 스스로 낫는다. 주문은
`_call_once` 라 재시도하지 않고 예외가 호출부로 올라간다.
...
모의투자 계좌로 실제 주문을 내고 확인한 값이다.

| 확인한 것 | 값 |
|---|---|
| 성공 응답의 주문번호 | **`ord_no`** 가 맞다. 7자리 제로패딩 (`"0018727"`) |
| 시장가 주문의 `ord_pric` | 응답에 `"0"` 으로 온다. `trde_tp` 는 `"시장가"` |
| `ord_uv` 형식 | **정수만.** `"-182000"` 처럼 부호가 붙으면 `1517` 로 거부 |
| 부분체결의 `ord_stt` | **`"체결"`.** 완전체결과 구분되지 않는다 |
| 한 주문의 행 수 | **한 행.** 미체결·체결 목록에 각각 하나씩만 |
| 취소할 잔량이 없을 때 | `[2000](RC4033:모의투자 정정/취소할 수량이 없습니다.)` |

**`ord_stt` 로 상태를 판정하지 않는 결정이 실측으로 뒷받침됐다.**
서원 1766주 중 1326주만 체결된 상태에서도 `ord_stt` 가 `"체결"` 이었다.
상태는 `ord_qty` 와 `cntr_qty` 로 끌어낸다.

**모의투자는 장 시작 전 주문을 받지 않는다.**

```
08:28 시장가 매수 → [2000](RC4057:모의투자 장시작전)
09:10 지정가 매수 → 접수
```

**동시호가(08:30~09:00)에 주문을 낼 수 없다.** 실전도 같은지는 확인하지
못했다 — 메시지에 `모의투자` 가 붙어 있어 모의 환경의 게이트로 보이지만
단정하지 않는다.

**취소 — 15:30 이후에는 안 된다** (2026-09-03 실측).

| 시각 | 결과 |
|---|---|
| 09:15 | `모의투자 취소주문완료` (`return_code` 0) |
| 15:31 · 15:45 · 16:10 | `[2000](RC4058:모의투자 장종료)` |

장중에는 되고 마감 뒤에는 막힌다. 엔진의 미체결 취소를 **15:10** 으로
잡은 근거다 (15:20 부터 장 마감 동시호가다).

취소 성공 응답은 `ord_no`(새 번호) · `base_orig_ord_no`(원 번호) ·
`cncl_qty` 를 준다. `cncl_qty` 는 12자리 제로패딩이다.

**취소된 주문은 어느 목록에도 남지 않는다.** 미체결(`ka10075`)에서 빠지고
체결(`ka10076`)에도 없다. `get_order_status` 가 `PermanentError` 를 던진다.
**체결 목록은 당일 것만 주므로 날이 바뀐 주문도 같다.**

재시작 복구가 이 예외로 죽지 않아야 한다 — 엔진은 그 주문을 `cancelled`
로 닫고 넘어간다. 보유 수량은 증권사 잔고가 정본이라 포지션 대조가 바로
뒤에 맞춘다.

**미체결 상태의 `ord_stt` 는 `"접수"` 다** (2026-09-03 실측). 부분체결·
완전체결이 `"체결"` 이므로, 어휘로는 접수와 체결만 구분된다.

**슬리피지 — 개장 정각 시장가** (2026-09-02·09-03 실측, 삼성전자 1주).
...
**09:00 정각이 이틀 연속 시가와 정확히 같다.** 모의투자 체결 시뮬레이터가
09:00 주문을 시가로 처리하는 것으로 보인다. 실전 근거로 쓰지 않는다.

쓸 수 있는 것은 **2분이 0.2~0.3%** 라는 것이다. 제출 시각이 값을 정한다.

**비용 실측** — 삼성전자 1주 시장가 매도, 체결가 260,250원.

| 항목 | 값 | 비율 |
|---|---|---|
| `tdy_trde_cmsn` | 910 | 0.35% (모의투자 요율) |
| `tdy_trde_tax` | 520 | **0.1998%** |

**세금이 `limits.yaml` 의 `tax_rate` 0.0020 과 일치한다.** 백테스트 비용
가정 하나가 실측으로 검증됐다. 수수료는 모의투자가 실전(0.015%)의 23배라
손익 대조에는 쓸 수 없다 (2026-08-31 기록 참조).
...
한쪽 끝만 위험한 지표는 **두 점**이다. `danger`에서 -1, `safe`에서 +1,
사이는 선형이고 밖은 잘라낸다. 값이 클수록 위험하면 `danger > safe`로 적는다.

```yaml
thresholds: { danger: 84.0, safe: 21.0 }    # VKOSPI
```

**양끝이 다 위험한 지표는 세 점이다** (2026-09-08 추가).

```yaml
thresholds: { danger_low: 0.0, safe: 20.0, danger_high: 60.0 }
```

`safe`를 꼭짓점으로 양쪽이 내려가는 삼각형이다. 이격도가 이 형식을 쓴다 —
낮으면 하락추세고 높으면 과열이라 직선으로는 그릴 수 없다. 두 점이던
시절 2026-05~06의 이격도 68%를 '최대 안전'으로 읽었고 7월에 -2.2%까지
무너졌다.

구현은 두 점 계산을 두 번 부른다. 윗가지는 `danger`가 더 큰 쪽으로 부르면
분모가 음수가 되어 방향이 저절로 뒤집힌다. **새 계산식을 만들지 않는다.**
...
| GET | `/api/disclosures?limit=` | 최근 전자공시 |
...
| 모의투자의 **주문** API 지원 | 조사했으나 명시적 근거를 못 찾았다 (2026-08-30). 규격은 확인됨 — `/api/dostk/ordr`, `kt10000`/`kt10001`. 조회 API 로 먼저 검증한다 |
