...
SYSTEM = """너는 한국 주식 시장 글에서 **테마 키워드**를 뽑는다.

이 키워드는 "요즘 어느 테마가 뜨는가" 를 세는 데 쓰인다.
절대 건수가 아니라 **평소보다 몇 배 나왔는가**가 신호다.

뽑기 전에 이것을 묻는다
- **이 표현으로 종목 여러 개를 묶을 수 있는가?** 못 묶으면 뽑지 않는다
  - 유리기판 -> 묶인다. 그 소재를 하는 회사들이 있다
  - 60GWh -> 못 묶는다. 한 회사의 수치다
  - 자사주 소각 -> 못 묶는다. 업종과 무관하게 아무 데나 붙는다
  - 인프라 -> 못 묶는다. 너무 넓어 아무 회사나 들어온다

뽑는 것
- 테마·기술·소재·제품 (HBM, 유리기판, 폴더블, 휴머노이드, 소형모듈원전)
- 원문에 나온 표현 그대로 쓴다. 새로 만들지 않는다
- 글마다 0개에서 3개. **확신이 없으면 빈 배열로 둔다**

뽑지 않는 것
- 종목명과 회사명. 약칭도 마찬가지다 (NV, 삼전, 닉스)
- 시황 표현 (급등, 상승, 하락, 특징주, 강세)
- **사건·행위** (자사주 소각, 유상증자, 라이선스아웃, 특허 회피, 계약 체결,
  수주, 실적 발표, 목표주가 상향)
- **수치·규격** (60GWh, 1조원, 전년 대비 20%, 3분기)
- **범용 명사** (인프라, 플랫폼, 솔루션, 시장, 산업, 사업, 매출, 이익)
- 두 글자 미만. 한 글자는 다른 낱말 안에 걸려 못 쓴다

넓은 말과 좁은 말이 같이 나오면 **좁은 것만** 뽑는다
- 'AI 반도체 HBM 수요' -> HBM 만. AI 와 반도체는 버린다
- '2차전지 양극재 단가' -> 양극재 만
- 넓은 말만 있고 좁은 말이 없으면 그때는 넓은 말을 뽑는다
...
# **한 글자는 받지 않는다.** 낱말 경계로도 못 막는다 — '금리' 는 '금' 이
# 맨 앞이라 왼쪽 경계가 깨끗하고 오른쪽은 조사와 구분이 안 된다.
# 2026-09-01 에 '금' 이 신주발행금지·금리·현금에 걸려 하루 26회가 됐다.
#
# 분석기도 같은 값으로 사전을 거른다 (collect.yaml 의 min_term_length).
# 여기서 막는 것은 애초에 들어오지 않게 하는 것이다
MIN_TERM_LENGTH = 2
...
def clean_terms(terms: list[str]) -> list[str]:
    ...
    for term in terms:
        ...
        if MIN_TERM_LENGTH <= len(cleaned) <= MAX_TERM_LENGTH and cleaned not in seen:
            seen.append(cleaned)
    return seen
