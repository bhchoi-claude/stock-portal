...
def test_프롬프트가_묶임을_기준으로_준다():
    """**이 표현으로 종목을 묶을 수 있는가.**

    '테마만 뽑아라' 로는 모델이 경계를 못 정한다. 60GWh(수치),
    자사주 소각(사건), 인프라(범용어)가 다 들어왔다 (2026-09-04).
    """
    assert "종목 여러 개를 묶을 수 있는가" in SYSTEM
    # 지적받은 셋이 예시로 들어 있다
    for bad in ("60GWh", "자사주 소각", "인프라"):
...
def test_프롬프트가_넓은_말을_버리라고_말한다():
    ...
    assert "두 글자 미만" in SYSTEM


def test_프롬프트가_확신_없으면_비우라고_말한다():
    """LLM 이 뽑은 표현은 사전에 영구히 남는다. 재현율보다 정밀도다."""
    assert "확신이 없으면" in SYSTEM
